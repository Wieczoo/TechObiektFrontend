import {
  require_prop_types,
  require_react_is
} from "./chunk-FPVLTH4P.js";
import {
  require_jsx_runtime
} from "./chunk-WQQ6HNJI.js";
import {
  _extends,
  _objectWithoutPropertiesLoose
} from "./chunk-RT7TDEPI.js";
import {
  require_react
...
function getFunctionName(fn) {
  const match3 = `${fn}`.match(fnNameMatchRegex);
  const name = match3 && match3[1];
  return name || "";
}
...
function focusTriggersKeyboardModality(node3) {
  const {
    type,
    tagName
  } = node3;
  if (tagName === "INPUT" && inputTypesWhitelist[type] && !node3.readOnly) {
    return true;
  }
  if (tagName === "TEXTAREA" && !node3.readOnly) {
    return true;
  }
  if (node3.isContentEditable) {
    return true;
  }
  return false;
}
...
function useIsFocusVisible() {
  const ref = React7.useCallback((node3) => {
    if (node3 != null) {
      prepare(node3.ownerDocument);
    }
  }, []);
...
var StyleSheet = function() {
  ...
  _proto.insert = function insert(rule) {
    ...
    if (true) {
      var isImportRule5 = rule.charCodeAt(0) === 64 && rule.charCodeAt(1) === 105;
      if (isImportRule5 && this._alreadyInsertedOrderInsensitiveRule) {
        console.error("You're attempting to insert the following rule:\n" + rule + "\n\n`@import` rules must be before all other types of rules in a stylesheet but other rules have already been inserted. Please ensure that `@import` rules are before all other rules.");
      }
      this._alreadyInsertedOrderInsensitiveRule = this._alreadyInsertedOrderInsensitiveRule || !isImportRule5;
    }
    if (this.isSpeedy) {
    ...
    switch (previous = character3, character3 = next()) {
        ...
        switch (character3) {
          ...
          case 59 + offset:
            ...
            if (property > 0 && strlen(characters3) - length3)
              append(property > 32 ? declaration(characters3 + ";", rule, parent, length3 - 1) : declaration(replace(characters3, " ", "") + ";", rule, parent, length3 - 2), declarations);
            break;
          case 59:
            characters3 += ";";
          default:
            append(reference = ruleset(characters3, root, parent, index, offset, rules, points, type, props = [], children = [], length3), rulesets);
            if (character3 === 123)
              if (offset === 0)
                parse(characters3, root, reference, reference, props, rulesets, length3, points, children);
              else
                switch (atrule === 99 && charat(characters3, 3) === 110 ? 100 : atrule) {
                  ...
                  case 109:
                  case 115:
                    parse(value, reference, reference, rule && append(ruleset(value, reference, reference, 0, 0, rules, points, type, rules, props = [], length3), children), rules, children, length3, points, rule ? props : children);
                    break;
                  default:
                    parse(characters3, reference, reference, reference, [""], children, 0, points, children);
                }
        }
        index = offset = property = 0, variable = ampersand = 1, type = characters3 = "", length3 = pseudo;
        break;
      case 58:
      ...
      default:
        ...
        switch (characters3 += from(character3), character3 * variable) {
          case 38:
            ampersand = offset > 0 ? 1 : (characters3 += "\f", -1);
            break;
          case 44:
          ...
          case 64:
            ...
            atrule = peek(), offset = length3 = strlen(type = characters3 += identifier(caret())), character3++;
            break;
          case 45:
...
function ruleset(value, root, parent, index, offset, rules, points, type, props, children, length3) {
  var post = offset - 1;
...
var toRules = function toRules2(parsed, points) {
  ...
  var character3 = 44;
  do {
    switch (token(character3)) {
      case 0:
        if (character3 === 38 && peek() === 12) {
          points[index] = 1;
        }
        parsed[index] += identifierWithPointTracking(position - 1, points, index);
        break;
      case 2:
        parsed[index] += delimit(character3);
        break;
      case 4:
        if (character3 === 44) {
      ...
      default:
        parsed[index] += from(character3);
    }
  } while (character3 = next());
  return parsed;
};
...
var Global = withEmotionCache(function(props, cache2) {
  ...
  if (!isBrowser2) {
    ...
    var next3 = serialized.next;
    while (next3 !== void 0) {
      serializedNames += " " + next3.name;
      serializedStyles += next3.styles;
      next3 = next3.next;
    }
...
var registerStyles3 = function registerStyles4(cache2, serialized, isStringTag2) {
  var className = cache2.key + "-" + serialized.name;
  if (
    // we only need to add the styles to the registered cache if the
    // class name could be used further down
    ...
    // the registered cache to know whether a style is global or not
    // also, note that this check will be dead code eliminated in the browser
    isBrowser3 === false) && cache2.registered[className] === void 0
  ) {
    cache2.registered[className] = serialized.styles;
  }
};
var insertStyles3 = function insertStyles4(cache2, serialized, isStringTag2) {
  registerStyles3(cache2, serialized, isStringTag2);
  var className = cache2.key + "-" + serialized.name;
  if (cache2.inserted[serialized.name] === void 0) {
    var current = serialized;
    do {
...
var processStyleName2 = memoize3(function(styleName) {
...
var processStyleValue3 = function processStyleValue4(key, value) {
  switch (key) {
    ...
    case "animationName": {
      if (typeof value === "string") {
        return value.replace(animationRegex2, function(match3, p1, p2) {
          cursor2 = {
            name: p1,
            styles: p2,
            next: cursor2
...
function handleInterpolation2(mergedProps, registered, interpolation) {
  ...
  switch (typeof interpolation) {
    ...
    case "string":
      if (true) {
        var matched = [];
        var replaced = interpolation.replace(animationRegex2, function(match3, p1, p2) {
          var fakeVarName = "animation" + matched.length;
          matched.push("const " + fakeVarName + " = keyframes`" + p2.replace(/^@keyframes animation-\w+/, "") + "`");
          return "${" + fakeVarName + "}";
        });
...
var serializeStyles3 = function serializeStyles4(args, registered, mergedProps) {
  ...
  var sourceMap;
  if (true) {
    styles = styles.replace(sourceMapPattern2, function(match4) {
      sourceMap = match4;
      return "";
    });
  }
  ...
  var identifierName = "";
  var match3;
  while ((match3 = labelPattern2.exec(styles)) !== null) {
    identifierName += "-" + // $FlowFixMe we know it's not null
    match3[1];
  }
...
var Insertion5 = function Insertion6(_ref) {
  ...
  registerStyles3(cache2, serialized, isStringTag2);
  useInsertionEffectAlwaysWithSyncFallback(function() {
    return insertStyles3(cache2, serialized, isStringTag2);
  });
  return null;
};
var createStyled = function createStyled2(tag, options) {
  ...
  return function() {
    ...
    var Styled = withEmotionCache(function(props, cache2, ref) {
      ...
      if (typeof props.className === "string") {
        className = getRegisteredStyles2(cache2.registered, classInterpolations, props.className);
      } else if (props.className != null) {
        className = props.className + " ";
      }
      var serialized = serializeStyles3(styles.concat(classInterpolations), cache2.registered, mergedProps);
      className += cache2.key + "-" + serialized.name;
...
// node_modules/@mui/styled-engine/node_modules/@emotion/sheet/dist/emotion-sheet.browser.esm.js
...
var character2 = 0;
var characters2 = "";
function node2(value, root, parent, type, props, children, length3) {
  return { value, root, parent, type, props, children, line: line2, column: column2, length: length3, return: "" };
}
      ...
      case 0:
        switch (character3) {
          ...
          case 59:
            characters3 += ";";
          default:
            append2(reference = ruleset2(characters3, root, parent, index, offset, rules, points, type, props = [], children = [], length3), rulesets);
            if (character3 === 123)
              if (offset === 0)
                parse2(characters3, root, reference, reference, props, rulesets, length3, points, children);
              else
                switch (atrule === 99 && charat2(characters3, 3) === 110 ? 100 : atrule) {
                  ...
                  case 109:
                  case 115:
                    parse2(value, reference, reference, rule && append2(ruleset2(value, reference, reference, 0, 0, rules, points, type, rules, props = [], length3), children), rules, children, length3, points, rule ? props : children);
                    break;
                  default:
      ...
      default:
        ...
        switch (characters3 += from2(character3), character3 * variable) {
          ...
          case 64:
            ...
            atrule = peek2(), offset = length3 = strlen2(type = characters3 += identifier2(caret2())), character3++;
            break;
          case 45:
...
function ruleset2(value, root, parent, index, offset, rules, points, type, props, children, length3) {
  var post = offset - 1;
  var rule = offset === 0 ? rules : [""];
  var size = sizeof2(rule);
  for (var i = 0, j = 0, k = 0; i < index; ++i)
    for (var x = 0, y = substr2(value, post + 1, post = abs2(j = points[i])), z = value; x < size; ++x)
      if (z = trim2(j > 0 ? rule[x] + " " + y : replace2(y, /&\f/g, rule[x])))
        props[k++] = z;
  return node2(value, root, parent, offset === 0 ? RULESET2 : type, props, children, length3);
}
function comment2(value, root, parent) {
  return node2(value, root, parent, COMMENT2, from2(char2()), substr2(value, 2, -2), 0);
}
function declaration2(value, root, parent, length3) {
  return node2(value, root, parent, DECLARATION2, substr2(value, 0, length3), substr2(value, length3 + 1, -1), length3);
}
...
var identifierWithPointTracking3 = function identifierWithPointTracking4(begin, points, index) {
  var previous = 0;
  var character3 = 0;
  while (true) {
    previous = character3;
...
var toRules3 = function toRules4(parsed, points) {
  var index = -1;
  var character3 = 44;
  do {
    switch (token2(character3)) {
      case 0:
        ...
        parsed[index] += identifierWithPointTracking3(position2 - 1, points, index);
        break;
      case 2:
...
var getRules3 = function getRules4(value, points) {
  return dealloc2(toRules3(alloc2(value), points));
};
var fixedElements2 = /* @__PURE__ */ new WeakMap();
var compat3 = function compat4(element) {
  if (element.type !== "rule" || !element.parent || // positive .length indicates that this rule contains pseudo
  // negative .length indicates that this rule has been already prefixed
  element.length < 1) {
    return;
  ...
  if (element.props.length === 1 && value.charCodeAt(0) !== 58 && !fixedElements2.get(parent)) {
    return;
  }
  if (isImplicitRule) {
    return;
  }
  fixedElements2.set(element, true);
  var points = [];
  var rules = getRules3(value, points);
  var parentRules = parent.props;
  for (var i = 0, k = 0; i < rules.length; i++) {
    for (var j = 0; j < parentRules.length; j++, k++) {
      element.props[k] = points[i] ? rules[i].replace(/&\f/g, parentRules[j]) : parentRules[j] + " " + rules[i];
...
var removeLabel3 = function removeLabel4(element) {
  if (element.type === "decl") {
    var value = element.value;
    if (
      // charcode for l
...
var ignoreFlag2 = "emotion-disable-server-rendering-unsafe-selector-warning-please-do-not-use-this-the-warning-exists-for-a-reason";
var isIgnoringComment3 = function isIgnoringComment4(element) {
  return element.type === "comm" && element.children.indexOf(ignoreFlag2) > -1;
};
var createUnsafeSelectorsAlarm3 = function createUnsafeSelectorsAlarm4(cache2) {
  return function(element, index, children) {
    if (element.type !== "rule" || cache2.compat)
      return;
    var unsafePseudoClasses = element.value.match(/(:first|:nth|:nth-last)-child/g);
...
var isImportRule3 = function isImportRule4(element) {
  return element.type.charCodeAt(1) === 105 && element.type.charCodeAt(0) === 64;
};
var isPrependedWithRegularRules3 = function isPrependedWithRegularRules4(index, children) {
  for (var i = index - 1; i >= 0; i--) {
    if (!isImportRule3(children[i])) {
      return true;
    }
  }
  return false;
};
var nullifyElement3 = function nullifyElement4(element) {
  element.type = "";
  element.value = "";
  element["return"] = "";
  element.children = "";
...
var incorrectImportAlarm3 = function incorrectImportAlarm4(element, index, children) {
  if (!isImportRule3(element)) {
    return;
  }
  if (element.parent) {
    console.error("`@import` rules can't be nested inside other rules. Please move it to the top level and put it before regular rules. Keep in mind that they can only be used within global styles.");
    nullifyElement3(element);
  } else if (isPrependedWithRegularRules3(index, children)) {
    console.error("`@import` rules can't be after other rules. Please put your `@import` rules before your other rules.");
    nullifyElement3(element);
  }
};
function prefix4(value, length3) {
    ...
    case 4765:
      if (strlen2(value) - 1 - length3 > 6)
        switch (charat2(value, length3 + 1)) {
          ...
          case 115:
            return ~indexof2(value, "stretch") ? prefix4(replace2(value, "stretch", "fill-available"), length3) + value : value;
        }
      break;
    case 4949:
...
var prefixer3 = function prefixer4(element, index, children, callback) {
  if (element.length > -1) {
    if (!element["return"])
      switch (element.type) {
        case DECLARATION2:
          element["return"] = prefix4(element.value, element.length);
          break;
        case KEYFRAMES2:
...
var defaultStylisPlugins2 = [prefixer3];
var createCache3 = function createCache4(options) {
  ...
  var stylisPlugins = options.stylisPlugins || defaultStylisPlugins2;
  if (true) {
    if (/[^a-z-]/.test(key)) {
      throw new Error('Emotion key must only contain lower case alphabetical characters and - but "' + key + '" was passed');
    }
  ...
  var _insert;
  var omnipresentPlugins = [compat3, removeLabel3];
  if (true) {
    omnipresentPlugins.push(createUnsafeSelectorsAlarm3({
      get compat() {
        return cache2.compat;
      }
    }), incorrectImportAlarm3);
  }
  {
    var currentSheet;
...
function mergeBreakpointsInOrder(breakpointsInput, ...styles) {
  ...
  const mergedOutput = [emptyBreakpoints, ...styles].reduce((prev3, next3) => deepmerge(prev3, next3), {});
...
var getCssProperties = memoize4((prop) => {
  if (prop.length > 2) {
    if (aliases[prop]) {
      prop = aliases[prop];
    } else {
...
function getValue(transformer, propValue) {
  ...
  const transformed = transformer(abs3);
  if (propValue >= 0) {
    return transformed;
  }
...
var positions_default = compose_default(position3, zIndex, top, right, bottom, left);
...
Please use another name.` : formatMuiErrorMessage(18));
  ...
  if (true) {
    ...
    const traverse = (node3, component) => {
      let key;
      for (key in node3) {
        ...
        if (stateClasses.indexOf(key) !== -1 && Object.keys(child).length > 0) {
          if (true) {
            ...
            console.error([`MUI: The \`${component}\` component increases the CSS specificity of the \`${key}\` internal state.`, "You can not override it like this: ", JSON.stringify(node3, null, 2), "", `Instead, you need to use the '&.${stateClass}' syntax:`, JSON.stringify({
              root: {
                [`&.${stateClass}`]: child
              }
...
@mui/private-theming/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-56P4MMVB.js.map
